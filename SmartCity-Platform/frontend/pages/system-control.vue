...
<template>
  <div class="space-y-6">
    ...
    <div class="glass-card p-8">
      <h3
        class="text-2xl font-semibold text-gray-900 dark:text-gray-200 mb-2 flex items-center gap-2"
      >
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        ...
        <div class="glass-card p-6 border border-primary/20">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              <h4
                class="text-lg font-semibold text-gray-900 dark:text-gray-200 mb-2"
              >
                Sync Data Now
              </h4>
              <p class="text-sm text-gray-400 mb-4">
                Manually trigger data synchronization from all edge nodes. This
                will pull the latest sensor data and update storage tiers.
              </p>
              <UiButton
                @click="handleSync"
                :disabled="isSyncing"
              ...
              >
                <div class="flex gap-2 items-center">
                  <RefreshCw
                    ...
                    class="w-5 h-5"
                  />
                  <p>{{ isSyncing ? "Syncing..." : "Trigger Sync" }}</p>
                </div>
              </UiButton>
            </div>
          </div>
        ...
        <div class="glass-card p-6 border border-red-500/20">
          <div class="flex items-start gap-4">
            ...
            <div class="flex-1">
              ...
              <p class="text-sm text-gray-400 mb-4">
                Clear all data from Redis and MongoDB. This action cannot be
                undone. Use for demo reset purposes only.
              </p>
              <UiButton
    ...
    <div class="glass-card p-6">
      <h3
        class="text-lg font-semibold text-gray-900 dark:text-gray-200 mb-4 flex items-center gap-2"
      >
      ...
      <div v-if="actionHistory.length > 0" class="space-y-3">
        <div
          ...
          :key="index"
          class="flex items-start gap-3 p-3 bg-dark-lighter/20 dark:bg-dark-lighter/60 rounded-lg"
        >
          <div
          ...
          />
          <div class="flex-1">
            <p class="text-sm font-medium text-gray-500">{{ action.action }}</p>
            <p class="text-xs text-gray-500 mt-1">{{ action.timestamp }}</p>
            <p
              v-if="action.message"
              class="text-xs mt-1"
              :class="action.success ? 'text-neon-green' : 'text-red-400'"
            >
              {{ action.message }}
            </p>
          </div>
      ...
      <div v-else class="text-center py-8 text-gray-500">No recent actions</div>
    </div>
        ...
        >
          <div class="glass-card p-6 max-w-md w-full border border-red-500/40">
            <div class="flex items-start gap-4 mb-6">
              ...
              <div>
                <h4
                  class="text-xl font-semibold text-gray-900 dark:text-gray-200 mb-2"
                >
                  Confirm System Reset
                </h4>
                <p class="text-sm text-gray-400">
                  This will permanently delete all data from HOT, WARM, and COLD
